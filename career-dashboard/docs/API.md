...
| PUT | `/pipeline/preferences` | Save `{count, source, provider, model, steps}`; also sets `/discovery/preferences`. |
| POST | `/pipeline/run` | Start a run with the same body (202). Refused while another run or a discovery is active, when today's plan is complete, or when the chosen AI is not ready. |
