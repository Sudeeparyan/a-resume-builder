...
2. If Annie says "give me 10 companies" (the `hunt` skill in `.agents/skills/hunt/`), run discovery first (tracked career pages, then AI discovery) and keep replacing excluded, already-seen, rejected, expired and duplicate leads until 10 distinct eligible live JDs are secured or the search is honestly exhausted.
