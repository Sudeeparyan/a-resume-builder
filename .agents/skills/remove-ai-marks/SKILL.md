...
## In this workspace: every resume and cover letter, built in

This skill runs on every resume the dashboard or an AI app makes, without the HTTP service below:
the app has its deterministic part built in (`career-dashboard/backend/ai_marks.py`). The rule
further down against cleaning locally is about improvising scripts; this built-in step is not that.

- **Layer A on text.** Every value written into `resume.tex` (`career.tex_escape`), every Resume
  Studio save and every cover letter drops hidden characters (zero-width, direction controls, BOM,
  tag characters, variation selectors), turns unusual spaces into plain ones and fixes a Cyrillic or
  Greek look-alike letter inside a Latin word. `career check-resume` fails a `resume.tex` that still
  has one and names it (`qa.json` → `ai_marks.source`): retype that word.
- **Container strip on the PDF.** Every compiled `resume.pdf` keeps only its Title and Author (the
  validator checks both against the candidate); producer, creator, subject, keywords, dates and any
  XMP packet are removed, and `career check-resume` fails a PDF that still has them
  (`qa.json` → `ai_marks.pdf`).
- **No Layer B on a resume or cover letter.** Their wording is the registered wording in
  `career-dashboard/data/context/evidence.yml`; a paraphrase or "humanize" pass would reword verified
  facts and the signature project, which Annie's rules forbid. Say so in one sentence instead of
  offering it.
- **Never send `resume.tex` to the service's `.tex` clean**: it strips `pdfauthor` and the
  `% EVIDENCE:` comments the validator needs. When the service answers `$WM/health`, you may
  `/inspect` the final `resume.pdf` as an extra check and report it; when it does not, the built-in
  step is enough. Do not stop the resume for it.
- The `references/` files and the `service/` folder named below are not in this workspace.
